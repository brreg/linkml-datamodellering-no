...
2. Gjer endringar og valider lokalt:
   ...
   - `make mcp-linkml-validate SCHEMA=...` (brukar `validation_policy` frå build.yaml, eller overstyr med `POLICY=bronze`)
...
- Validere data før publisering (`make mcp-linkml-validate SCHEMA=...` — brukar automatisk `validation_policy` frå build.yaml)
