...
# Hoppa-over-eksempel vert logga til stderr. Sjølve linkml-convert-kallet
# gjer kallaren (ikkje dette scriptet), sidan det krev $(LINKML_RUN) sin
# podman-kontekst — den strengen inneheld sjølve anførselsteikn (frå
# WORK_MOUNT) som berre er trygge å la shellen tolke når dei kjem direkte
# frå ei Make-recipe-linje, ikkje via eit miljøvariabel-lag.
#
# Bruk: convert-examples.sh [domene]
# Miljøvariablar som må vere sette: SCHEMA_DIR, GEN_DIR
set -euo pipefail
...
for example in $(find "$search_dir" -path '*/examples/*-eksempel.yaml' 2>/dev/null | sort); do
    ...
    if [ -f "$manifest" ] && grep -q "^  example_rdf: false" "$manifest"; then
        echo "Hoppar over linkml-convert for $example (example_rdf: false)" >&2
        continue
    fi
