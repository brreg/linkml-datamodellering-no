# Changelog

## [2.1.1](https://github.com/brreg/linkml-datamodellering-no/compare/referansemodell-gold-v2.1.0...referansemodell-gold-v2.1.1) (2026-09-01)


### Bug Fixes

* **referanse:** fjern alle valideringsfunn i referansemodellane ([c90728b](https://github.com/brreg/linkml-datamodellering-no/commit/c90728bea35e451a144e64389474f667606a6167))

## [2.1.0](https://github.com/brreg/linkml-datamodellering-no/compare/referansemodell-gold-v2.0.1...referansemodell-gold-v2.1.0) (2026-08-25)


### Features

* **gen-java:** legg til make gen-java og Java-bruk-seksjon i Kom i gang ([74396fc](https://github.com/brreg/linkml-datamodellering-no/commit/74396fcfb1531244e32d27599bf09353381e3fb1))

## [2.0.1](https://github.com/brreg/linkml-datamodellering-no/compare/referansemodell-gold-v2.0.0...referansemodell-gold-v2.0.1) (2026-08-23)


### Bug Fixes

* **referanse:** legg til manglande annotations og status-badge-guard ([41595bc](https://github.com/brreg/linkml-datamodellering-no/commit/41595bcc95f72b01241a12e716365dcaea9834d7))

## [2.0.0](https://github.com/brreg/linkml-datamodellering-no/compare/referansemodell-gold-v1.0.0...referansemodell-gold-v2.0.0) (2026-08-17)


### ⚠ BREAKING CHANGES

* **referansemodell-gold:** slot "namn" er endra til "navn" i klassen Aktor.

### Features

* **generatorar:** legg til gen-graphql for GraphQL SDL-generering ([cb0a7bd](https://github.com/brreg/linkml-datamodellering-no/commit/cb0a7bd4c220dbee889df488bed0eaeb25960ce4))


### Bug Fixes

* **build-manifest:** legg til manglande generator-flagg i 11 build.yaml ([916d9df](https://github.com/brreg/linkml-datamodellering-no/commit/916d9df3205e7b1cb322eb8bdc8a59ce4d692a5b))
* **referansemodell-gold:** rett namn-slot til navn i klassen Aktor ([1d97c1b](https://github.com/brreg/linkml-datamodellering-no/commit/1d97c1b28a4e3566e1bc6840e0530b976e5464b1))


### Performance Improvements

* **mcp-linkml-validator:** parallelliser validate-examples, regenerer valideringsloggar ([fc67279](https://github.com/brreg/linkml-datamodellering-no/commit/fc672798d9b85870590a1bf7ea33fbca4181bf51))
