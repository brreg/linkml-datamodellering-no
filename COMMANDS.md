...
| Kommando | Beskriving | Output |
|---|---|---|
| `make new-modell DOMAIN=<domene> NAME=<modell> [JSON_SCHEMA=<sti>] [SKIP_EXAMPLE=1]` | Opprettar katalogstruktur og boilerplate for ein ny LinkML-domenemodell. Utan `JSON_SCHEMA` genererast eit tomt stub-skjema (`--input-format empty`). Med `JSON_SCHEMA=<sti til eksportert JSON Schema>` genererast skjemaet frå JSON Schema-innhaldet i staden (same konvertering som `make mcp-linkml-modell-utkast`, men resultatet landar direkte i den nye katalogstrukturen med full etterbehandling — id/navn/tittel, `annotations.utgiver`/dato, versjonslåst import). Eksempeldatafila vert i begge tilfelle fylt med eit rikt syntetisk datasett — alle slots (ikkje berre obligatoriske/identifikator), kryssreferansar mellom genererte instansar (fell tilbake til ein minimal éin-linjes stub dersom automatisk generering feilar). `SKIP_EXAMPLE=1` hoppar over eksempeldatagenereringa heilt (ingen podman-kall til `mcp-linkml-modell-utkast`, dermed ingen nettverksavhengd importoppløysing) og skriv direkte den same minimale éin-linjes stubben — nyttig for fullstendig offline scaffolding (t.d. `javazone-demo-script.sh`). | `src/linkml/<domain>/<modell>/<modell>-schema.yaml`<br>`src/linkml/<domain>/<modell>/examples/<modell>-eksempel.yaml` |
