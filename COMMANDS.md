...
| <a id="validate-linkml-merge"></a>`make validate-linkml-merge [DOMAIN=...] [SCHEMA=...]` | Same underliggande validering som `make validate`, men med `DOMAIN=`/`SCHEMA=`-støtte — del av Fase 1 i `domain-*`-pipelinen (steg 1, sjå § "Generering av artefakter"), sjeldan kalla frittståande. Ingen fil skriven (fail-fast validering). | Validerings-resultat per skjema til stdout | Batcha — `batch-generate.py --generator merge`, éin kontainar |
