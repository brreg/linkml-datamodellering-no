...
generate_submodel_box() {
    ...
    local container_parent_schema
    container_parent_schema=$(to_container_path "$parent_schema")
    local parent_title
    parent_title=$(run_python_container -c "import yaml; d=yaml.safe_load(open('$container_parent_schema')); print(d.get('title', d.get('name', '$parent')))" 2>&1) \
...
generate_submodels_section() {
    ...
    for sub in $submodels; do
        ...
        local container_sub_schema
        container_sub_schema=$(to_container_path "$sub_schema")
        local sub_title
        sub_title=$(run_python_container -c "import yaml; d=yaml.safe_load(open('$container_sub_schema')); print(d.get('title', d.get('name', '$sub')))" 2>&1) \
            || { echo "ÅTVARING: kunne ikkje lese title frå $sub_schema — bruker '$sub' ($sub_title)" >&2; sub_title="$sub"; }
        local sub_desc
        sub_desc=$(run_python_container -c "import yaml; d=yaml.safe_load(open('$container_sub_schema')); desc=d.get('description', ''); print(desc.split('.')[0] if desc else '')" 2>&1) \
